function [lp,dlp,d2lp,d3lp] = logphiLapse(z,delta)

% Copyright (c) by Carl Edward Rasmussen and Hannes Nickisch, 2013-11-13.
% logphi but incorporating a lapse rate to be used by likErfLapse
%
% The function is based on index 5725 in Hart et al. and gsl_sf_log_erfc_e.
%
% Copyright (c) by Carl Edward Rasmussen and Hannes Nickisch, 2013-11-13.
%
% Lapse rate introduced by Josef Schlittenlacher, 2016-06-30.
% does not use the safe computation. Required? Why at close to zero?
...
%    2*(1-pi/3); (4-pi)/3; 1; 1];
% f = 0; for i=1:14, f = lp0.*(c(i)+f); end
% lp(id1) = -2*f-log(2);
% 
...
% 
% id3 = ~id2 & ~id1;
% lp(id3) = log(erfc(-z(id3)/sqrt(2))/2);  % third case: rest

lp = log( normcdf( z ) * (1-2*delta) + delta );
...
dlp = (1-2*delta)*normpdf(z) ./ ( (1-2*delta)*normcdf(z) + delta );
