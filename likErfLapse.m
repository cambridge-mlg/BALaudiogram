function [varargout] = likErfLapse(hyp, y, mu, s2, inf, i)
...
% calls logphiLapse instead of logphi.
%
...
if nargin<5                              % prediction mode if inf is not present
  ...
  if s2zero                                         % log probability evaluation
    ...
    lp = likErfLapse(hyp, y, mu, s2, 'infEP');
  end
  p = exp(lp); ymu = {}; ys2 = {};
  if nargout>1
  ...
  case 'infLaplace'
    if nargin<6                                             % no derivative mode
      ...
      varargout = cell(nargout,1); [varargout{:}] = logphiLapse(yf);   % query logphiLapse
      if nargout>1
  ...
  case 'infEP'
    if nargin<6                                             % no derivative mode
      ...
      if nargout<=1, lZ = logphiLapse(z);                         % log part function
      else          [lZ,n_p] = logphiLapse(z); end
      if nargout>1
        if numel(y)==0, y=1; end
